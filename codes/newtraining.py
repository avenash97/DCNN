...
def get_unet(lr=1e-4, deep=True, dims=20, conv_channel=32, N_Cls=10, bn=False, use_sample_weights=True,
             init="glorot_uniform", dropout=False, big=True,use_jaccard_loss=False):
    """
    ...
    for iteration in range(num_iterations):
        ...
        if LR_decay:
            if iteration > 0:
                ...
                new_LR = float(K.get_value(model.optimizer.lr) * LR_decay)
                K.set_value(model.optimizer.lr,new_LR)
        print(K.get_value(model.optimizer.lr))
#	print("hello"+model.optimizer.lr.get_value())
        print("-------------------------------------------------------------------------------------")
        logger.info("LR: {}".format(K.get_value(model.optimizer.lr)))
        logger.info("-------------------------------------------------------------------------------------")
        if use_sample_weights:
