...
from newtraining import get_unet
...
def predict_id(id, model, trs, dims, size=1600, mins=None, maxs=None, use_sample_weights=True, raw=False, means=None):
    """
...
def predict_test(max_score="", model=None, trs=None, size=1600, mins=None, maxs=None, dims=20, raw=True, means=None):
    ...
    for i, id in enumerate(sorted(set(SB['ImageId'].tolist()))):
        ...
        np.save('../msks/{}_{}'.format(max_score, id), msk)
...
if __name__ == "__main__":
    ...
    scores = [float(j[-6:]) for j in os.listdir("../weights") if "hdf5" not in j]
    ...
    predict_test(max_score, model=model,mins=mins,maxs=maxs)
    make_submit("subm_1", max_score,trs)
